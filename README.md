...
После запуска приложения вы можете изучить и протестировать API через интерактивную документацию.

- **Запустите `user-service`**  
`mvn spring-boot:run` или через вашу IDE.  
- **Откройте Swagger UI**  
Перейдите по адресу: https://localhost:9497/swagger-ui.html  
Примите риск самоподписанного сертификата (если браузер предупредит).  
- **Авторизуйтесь**  
Нажмите кнопку "Authorize" (в правом верхнем углу).  
Введите учётные данные администратора:  
Username: `admin@demo.ya`  
Password: `admin123`  
Затем нажмите Authorize и закройте окно.  
- **Выполните любой запрос**  
Например, разверните раздел GET /api/users, нажмите "Try it out", затем "Execute".  
В ответе вы увидите не только данные пользователей, но и HATEOAS-ссылки (_links),  
которые позволяют навигировать по API.  
- **Проверьте HATEOAS**  
Пример ответа при получении списка пользователей:
```json
{
  "_embedded": {
    "userResponseList": [
      {
        ...
        "name": "Admin",
        "email": "admin@demo.ya",
        "_links": { "self": { "href": "http://localhost/api/users/1" } }
      }
    ]
  },
  "_links": {
    "self": { "href": "http://localhost/api/users" },
    "create": { "href": "http://localhost/api/users" }
  }
}
```  
Поле `_links` помогает клиенту понять, какие действия возможны с ресурсом.  
